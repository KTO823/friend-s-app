...
function switchTab(tabId) {
  ...
  // 導覽列按鈕高亮
  document.querySelectorAll('nav button').forEach(b => b.style.color = '#a0a0a0');
  document.getElementById('nav-' + tabId).style.color = '#2c2c2c';

  const headerTitle = document.getElementById('main-header-title');
  if (tabId === 'birthday') { headerTitle.innerHTML = '生日倒數'; if (window.loadBirthdays) window.loadBirthdays(); } 
  else if (tabId === 'gift') { headerTitle.innerHTML = '許願與禮物'; } 
  else if (tabId === 'trip') { headerTitle.innerHTML = '行程代購'; } 
  else if (tabId === 'debt') { headerTitle.innerHTML = '結算中心'; }
  else if (tabId === 'settings') { headerTitle.innerHTML = '個人設定'; }
}
...
// === 原本的極簡化渲染邏輯 (簡化高彩度標籤) ===
// 由於篇幅限制，請確保 renderTrips 和 renderGifts 中的 <span class="tag"> 
// 都換成類似 <span class="tag-soft"> 或 <span class="tag-highlight"> 即可，
// 邏輯與之前完全相同，只是拿掉了 #3b82f6 等強烈色彩。